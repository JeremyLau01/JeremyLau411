...
def test_create_meal(mock_cursor):
    ...
    expected_arguments = ("Meal Name", "Cuisine Name", 20.24, "LOW")
    assert actual_arguments == expected_arguments, f"The SQL query arguments did not match. Expected {expected_arguments}, got {actual_arguments}."

def test_create_meal_duplicate(mock_cursor):
    """Test creating a meal with a duplicate meal argument (should raise an error)."""
...
def test_create_meal_invalid_price():
    ...
    with pytest.raises(ValueError, match="Invalid price: -180. Price must be a positive number."):
        create_meal(meal="Meal Name", cuisine="Cuisine Name", price=-180, difficulty="LOW")
...
def test_delete_meal_by_id(mock_cursor):
